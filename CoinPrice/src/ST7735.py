...
#   @micropython.native
  def text( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False, aBackColor = BLACK ) :
    ...
    for c in aString:
      self.char((px, py), c, aColor, aFont, wh, aBackColor)
      px += width
...
#   @micropython.native
  def char( self, aPos, aChar, aColor, aFont, aSizes, aBackColor = BLACK ) :
    ...
    if (startchar <= ci <= endchar):
      ...
      if aSizes[0] <= 1 and aSizes[1] <= 1 :
        buf = bytearray((aBackColor >> 8, aBackColor) * (fonth * fontw))
